  ...
  try {
    ...
    if (actualUserType === 'TENANT') {
      ...
      try {
        ...
        // Optimization: If structured criteria are found (e.g. city, price), 
        // clear the raw query to prevent double-filtering by keywords in search-service
        ...
        const searchCriteria = { 
          ...criteria, 
          query: hasStructuredFilters ? '' : query 
        }

        // 使用 searchRentalProperties 进行搜索，它已经包含了优化的内存过滤逻辑
        const results = await searchRentalProperties(searchCriteria, resolvedUserId)
