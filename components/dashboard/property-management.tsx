...
export function PropertyManagement({ userId }: { userId?: string }) {
  ...
  const fetchProperties = useCallback(async () => {
    try {
      ...
      let propertiesSource = toArray(data.properties)
      if (propertiesSource.length === 0) {
        propertiesSource = toArray(data.data)
      }
      ...
      if (propertiesSource.length === 0 && url.includes('landlordId')) {
         try {
            ...
            if (fallbackRes.ok && isMounted.current) {
              ...
              let fallbackProps = toArray(fallbackData.properties)
              if (fallbackProps.length === 0) {
                fallbackProps = toArray(fallbackData.data)
              }
              if (fallbackProps.length > 0) {
      ...
      const formattedProperties = (Array.isArray(propertiesSource) ? propertiesSource : []).map((p: any) => ({
