...
export function getAppRegion(): 'global' | 'china' {
  const rawRegion = (process.env.NEXT_PUBLIC_APP_REGION || 'global').trim().toLowerCase()
  if (['china', 'cn', 'zh', 'domestic'].includes(rawRegion)) {
    return 'china'
  }
  return 'global'
}
...
export class SupabaseAdapter implements DatabaseAdapter {
  ...
  private async executeWithReconnect<T>(operation: () => Promise<T>): Promise<T> {
    try {
  ...
  async findUserByEmail(email: string): Promise<UnifiedUser | null> {
    try {
      const user = await this.executeWithReconnect(() =>
  ...
  async query<T = any>(collection: string, filters?: any, options?: any): Promise<T[]> {
    ...
    const modelMap: Record<string, any> = {
      ...
      'testimonials': (prisma as any).testimonial,
    ...
    try {
      return await this.executeWithReconnect(
  ...
  async findById<T = any>(collection: string, id: string): Promise<T | null> {
    ...
    try {
      ...
      const actualTableName = columnsResult.length ? tableName : tableName.toLowerCase()
      const existingColumns = (fallbackColumns || []).map((col: any) => String(col.column_name))
      if (existingColumns.length === 0) {
        return null
      }
      const selectableColumns = existingColumns.map((col) => `"${col}"`).join(', ')
      const rows = await prisma.$queryRawUnsafe(
        `SELECT ${selectableColumns} FROM "${actualTableName}" WHERE "id" = $1 LIMIT 1`,
        id
      ) as any[]
  ...
  async create<T = any>(collection: string, data: any): Promise<T> {
    ...
    try {
      return await model.create({ data }) as Promise<T>
    } catch (error: any) {
      const errorMsg = String(error?.message || '')
      const lower = errorMsg.toLowerCase()
      const isColumnMissing =
      ...
      const actualTableName = columnsResult.length ? tableName : tableName.toLowerCase()
      const existingColumns = new Set((fallbackColumns || []).map((col: any) => String(col.column_name)))
      const columnTypeMap = new Map<string, string>(
        (fallbackColumns || []).map((col: any) => [
          String(col.column_name),
          String(col.udt_name || col.data_type || '').toLowerCase()
        ])
      )
      const insertData: Record<string, any> = { ...(data || {}) }
      if (existingColumns.has('id') && (insertData.id === undefined || insertData.id === null || insertData.id === '')) {
        const idType = columnTypeMap.get('id') || ''
        insertData.id = idType === 'uuid'
          ? (globalThis.crypto?.randomUUID?.() || `${Date.now()}-${Math.random().toString(36).slice(2, 12)}`)
          : `c${Date.now().toString(36)}${Math.random().toString(36).slice(2, 10)}`
      }
      if (existingColumns.has('createdAt') && (insertData.createdAt === undefined || insertData.createdAt === null)) {
        insertData.createdAt = new Date()
      }
      if (existingColumns.has('updatedAt') && (insertData.updatedAt === undefined || insertData.updatedAt === null)) {
        insertData.updatedAt = new Date()
      }
      const entries = Object.entries(insertData).filter(([key, value]) => {
        if (!existingColumns.has(key)) return false
      ...
      const placeholders = entries.map(([key], index) => {
        const columnType = columnTypeMap.get(key) || ''
        if (columnType === 'json') return `$${index + 1}::json`
        if (columnType === 'jsonb') return `$${index + 1}::jsonb`
        return `$${index + 1}`
      })
      const sql = `INSERT INTO "${actualTableName}" (${columns.join(', ')}) VALUES (${placeholders.join(', ')}) RETURNING *`
